...
SQLite database underneath a Core Data `.storedata` extension. The `calls_*`
tools open it read-only (`file:...?mode=ro`) and never write to it. Only the
columns these tools actually query are documented here.
...
| Column | Meaning |
|---|---|
| `Z_PK` | Core Data internal primary key. Not exposed by this plugin. |
...
| `ZISO_COUNTRY_CODE`, `ZLOCATION`, `ZFACE_TIME_DATA`, `ZDISCONNECTED_CAUSE`, `ZREAD` | Present in the real schema but not read by this plugin — out of scope for what this phase needs (geo/location data in particular is deliberately not surfaced). |
...
in the `imessage` skill's own `references/schema.md`. Dividing `ZDATE` by
`1_000_000_000` the way `chat.db` requires would produce a timestamp decades in the
past; `server.py`'s `core_data_ts_to_dt`/`dt_to_core_data_ts` do the plain-seconds
conversion instead. Confirmed against both `mac_apt` and `iLEAPP`'s own conversion
code (both add `978307200` directly to `ZDATE`, no nanosecond scaling) rather than
...
resolution is a separate lookup against the local AddressBook, shared with the
`imessage` skill's tools via the same `contacts.py` module (see the `imessage`
skill's `references/schema.md` for that database's layout).
