...
Two published, independent implementations were pulled and compared against five
real reference `attributedBody` blobs (embedded verbatim in
...
Both decoded the short/medium/emoji fixtures correctly. On a 277-character "long
...
legitimate trailing spaces the sender actually typed. daveremy's decoded all five
fixtures byte-for-byte correctly.

**Decision: ported daveremy's marker-scan + three-tier length algorithm**
(`extract_text_from_attributed_body` in `server.py`), with one change — no
`.strip()`/`.trim()` on the result, since trailing/leading whitespace can be real
message content and stripping it is exactly the bug that tripped up the
implementation we didn't pick. A community-wide survey of comparable MCP servers
(see the `icallhistory` skill's sibling research) confirms most mature
implementations layer a fuller typedstream parser first and fall back to a
marker-scan heuristic like this one for edge cases the strict parser can't handle
— this plugin only implements the heuristic layer, which is sufficient for the
common single-string case this tool targets.
...
- **Tapback reactions** also arrive as `attributedBody`-only messages with
  human-readable derived text (e.g. `Reacted 😒 to "No treats "`). They decode
  fine; they're filtered out of this plugin's default output by
  `associated_message_type != 0`, not by the decoder (see `schema.md`), and their
  `associated_message_type` code is separately decoded into a `reaction_type`
  field when `include_reactions=True`.
...
- **Malformed/truncated blobs**: return `None` and fall through to the `text` column,
  never raise. Covered directly by `tests/test_server.py`'s
  `TestAttributedBodyDecode`.
